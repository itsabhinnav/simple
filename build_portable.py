...
OLLAMA_RESOURCES_DIR = BACKEND_DIR / "resources" / "ollama"
...
def collect_ollama_data_entries(onefile: bool) -> list[tuple[str, str]]:
    """Return PyInstaller (src, dst) pairs for the bundled Ollama runtime.

    Expects ``backend/scripts/prepare_ollama_resources.ps1`` to have produced
    ``backend/resources/ollama/ollama.exe`` plus ``.../models/<blobs>``.
    Empty list if the directory is missing — the caller decides whether
    that's fatal.
    """
    if not OLLAMA_RESOURCES_DIR.is_dir():
        return []

    binary_name = "ollama.exe" if os.name == "nt" else "ollama"
    binary_path = OLLAMA_RESOURCES_DIR / binary_name
    entries: list[tuple[str, str]] = []
    if binary_path.exists():
        entries.append((binary_path.as_posix(), "resources/ollama"))

    models_root = OLLAMA_RESOURCES_DIR / "models"
    if models_root.is_dir():
        if onefile:
            print(
                "[build] WARNING: --with-ollama + --onefile will extract the "
                "entire model dir (multi-GB) on every launch. Prefer one-folder."
            )
        # PyInstaller's `datas` entries take a *file* on the LHS and a
        # *destination directory* on the RHS. Walk the staged blobs and
        # preserve the relative path under resources/ollama/models/.
        for path in models_root.rglob("*"):
            if not path.is_file():
                continue
            rel_dir = path.parent.relative_to(models_root).as_posix()
            dest_dir = "resources/ollama/models"
            if rel_dir and rel_dir != ".":
                dest_dir = f"{dest_dir}/{rel_dir}"
            entries.append((path.as_posix(), dest_dir))
    return entries


def write_spec(onefile: bool, seed_db: Path | None, with_ollama: bool) -> Path:
    ...
    ollama_data_lines = ""
    if with_ollama:
        ollama_entries = collect_ollama_data_entries(onefile)
        if not ollama_entries:
            sys.exit(
                f"[build] --with-ollama set but no payload found at {OLLAMA_RESOURCES_DIR}. "
                "Run backend/scripts/prepare_ollama_resources.ps1 first."
            )
        bundled_size = sum(
            (OLLAMA_RESOURCES_DIR / src.split("resources/ollama/", 1)[-1]).stat().st_size
            for src, _ in ollama_entries
            if Path(src).exists()
        ) / (1024 ** 3)
        print(f"[build] Bundling Ollama payload: {len(ollama_entries)} files, {bundled_size:.2f} GiB")
        ollama_data_lines = "".join(
            f"    (r'{src}', '{dst}'),\n" for src, dst in ollama_entries
        )
...
{seed_data_line}{ollama_data_lines}]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Bundle ollama.exe and the staged Qwen2.5-VL model blobs from "
            "backend/resources/ollama/ into the PyInstaller payload. Run "
            "backend/scripts/prepare_ollama_resources.ps1 first to produce "
            "that directory. Strongly recommended only with --onefile OFF "
            "(model blobs are multi-GB)."
        ),
    )
    return p.parse_args()
...
def main() -> int:
    ...
    spec = write_spec(args.onefile, seed_db, args.with_ollama)

...
export class DashboardComponent implements OnInit {
  ...
  totalAssets = computed(() =>
    this.requirementsCount() + this.testCasesCount() + this.specsCount()
  );
  ...
  /** Composite project-health score (0–100) based on test coverage. */
  qualityScore = computed(() => this.coveragePercent());
  ...
  /** Combined recent activity timeline across requirements / tests, newest first. */
  combinedActivity = computed(() => {
    const items: { kind: 'requirement' | 'test'; icon: string; id: string; title: string; meta: string; ts: number; route: string; color: string }[] = [];
    for (const r of this.recentRequirements()) {
      const ts = r.created_at ? new Date(r.created_at).getTime() : 0;
      items.push({ kind: 'requirement', icon: '📋', id: r.requirement_id, title: r.title || '(untitled)', meta: r.priority || '', ts, route: '/requirements', color: '#3b82f6' });
    }
  ...
  formatRelativeTime(ts: number): string {
    ...
    if (m < 1) return this.i18n.translateLiteral('just now');
    if (m < 60) return this.i18n.translateLiteral(`${m}m ago`);
    const h = Math.round(m / 60);
    if (h < 24) return this.i18n.translateLiteral(`${h}h ago`);
    const d = Math.round(h / 24);
    if (d < 30) return this.i18n.translateLiteral(`${d}d ago`);
    const mo = Math.round(d / 30);
    if (mo < 12) return this.i18n.translateLiteral(`${mo}mo ago`);
    return this.i18n.translateLiteral(`${Math.round(mo / 12)}y ago`);
  }
